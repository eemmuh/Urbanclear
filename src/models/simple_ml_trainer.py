...
class SimpleLinearRegression:
    ...
    def fit(self, X: List[List[float]], y: List[float]):
        ...
        for epoch in range(epochs):
            ...
            # Calculate loss
            loss = (
                sum((pred - actual) ** 2 for pred, actual in zip(predictions, y))
                / n_samples
            )
    ...
    def _build_tree(self, X: List[List[float]], y: List[float], depth: int) -> Dict:
        ...
        for feature in range(n_features):
            ...
            for i in range(len(unique_values) - 1):
                ...
                left_mse = sum(
                    (yi - sum(left_y) / len(left_y)) ** 2 for yi in left_y
                ) / len(left_y)
                right_mse = sum(
                    (yi - sum(right_y) / len(right_y)) ** 2 for yi in right_y
                ) / len(right_y)

                weighted_mse = (
                    len(left_y) * left_mse + len(right_y) * right_mse
                ) / len(y)
...
class SimpleMLTrainer:
    ...
    def train_traffic_predictor(self, samples: int = 1000) -> Dict[str, Any]:
        ...
        for name, model in models.items():
            ...
            mse = sum(
                (pred - actual) ** 2 for pred, actual in zip(predictions, y_test)
            ) / len(y_test)
            mae = sum(
                abs(pred - actual) for pred, actual in zip(predictions, y_test)
            ) / len(y_test)
    ...
    def train_incident_detector(self, samples: int = 1000) -> Dict[str, Any]:
        ...
        accuracy = sum(
            pred == actual for pred, actual in zip(binary_predictions, y_test)
        ) / len(y_test)
        ...
        logger.info(
            f"Incident detector trained successfully with accuracy: {accuracy:.3f}"
        )
        return metadata
    ...
    def train_route_optimizer(self, samples: int = 1000) -> Dict[str, Any]:
        ...
        for _ in range(samples):
            ...
            X.append(
                [distance, current_traffic, historical_average, weather, time_of_day]
            )
        ...
        logger.info(
            f"Route optimizer trained successfully - MSE: {mse:.2f}, MAE: {mae:.2f}"
        )
        return metadata
        ...
        for model_type in ["traffic_predictor", "incident_detector", "route_optimizer"]:
            ...
            if model_path.exists() and metadata_path.exists():
                try:
                    ...
                    status[model_type] = {"trained": True, "metadata": metadata}
                except Exception as e:
                    status[model_type] = {
                        "trained": False,
