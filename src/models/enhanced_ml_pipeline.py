...
class EnhancedMLPipeline:
    ...
    def __init__(self, experiment_name: str = "urbanclear_traffic_prediction"):
        ...
        logger.info(f"Enhanced ML Pipeline initialized for experiment: {experiment_name}")
    ...
    def train_model(self, 
                   ...
                   X: pd.DataFrame,
                   y: pd.Series,
                   optimize_hyperparameters: bool = True) -> ModelMetrics:
        ...
        if optimize_hyperparameters:
            best_params = self.optimize_hyperparameters(config.model_type, X_train, y_train)
        ...
        metrics = ModelMetrics(
            ...
            cv_score=cross_val_score(model, X_train_scaled, y_train, cv=config.cross_validation_folds).mean(),
        ...
        logger.info(f"Model {config.model_type} trained successfully. R² score: {metrics.r2:.4f}")
        return metrics
    ...
    def ensemble_prediction(self, 
        ...
        # Weighted ensemble
        weighted_predictions = np.average(predictions, axis=0, weights=weights[:len(predictions)])
    ...
    def model_comparison_report(self) -> pd.DataFrame:
        ...
        for model_type, model in self.models.items():
            ...
            comparison_data.append({
                ...
                'parameters': len(config.hyperparameters) if hasattr(config, 'hyperparameters') else 0,
                'features': len(config.feature_columns) if hasattr(config, 'feature_columns') else 0,
