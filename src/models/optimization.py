...
class RouteOptimizer:
    ...
    async def _calculate_alternative_routes(
        ...
        for i in range(num_alternatives):
            ...
            total_distance = (sum(point.distance_from_start for point in route_points[-1:]) or 5.2) * distance_modifier
            total_time = (sum(point.estimated_travel_time for point in route_points) or 18) * time_modifier
    ...
    def _generate_route_points(
        ...
        for i in range(num_points + 2):  # +2 to include origin and destination
            ...
            segment_distance = random.uniform(0.8, 1.5)  # Mock: 0.8-1.5 miles per segment
