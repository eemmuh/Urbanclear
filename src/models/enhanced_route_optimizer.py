...
try:
    ...
    logging.warning("NetworkX not available. Advanced graph algorithms will be limited.")
...
class EnhancedRouteOptimizer:
        ...
        try:
            ...
            if NETWORKX_AVAILABLE and self.road_network:
                for edge in self.road_network.edges(data=True):
                    ...
                    traffic_factor = traffic_data.get(edge_key, {}).get('traffic_factor', 1.0)
                    congestion_level = traffic_data.get(edge_key, {}).get('congestion_level', 0.0)
                    
                    # Update edge weights
                    base_time = edge_data['base_time']
                    adjusted_time = base_time * traffic_factor * (1 + congestion_level)
                    
                    self.road_network[start_node][end_node]['current_time'] = adjusted_time
                    self.road_network[start_node][end_node]['traffic_factor'] = traffic_factor
                    self.road_network[start_node][end_node]['congestion_level'] = congestion_level
    ...
    def optimize_route(self, request: RouteOptimizationRequest) -> OptimizedRoute:
        ...
        try:
            logger.info(f"Optimizing route from {request.origin.address} to {request.destination.address}")
            ...
            routes['time_optimal'] = self._optimize_for_time(origin_node, destination_node, request)
            
            # 2. Distance-optimized route
            routes['distance_optimal'] = self._optimize_for_distance(origin_node, destination_node, request)
            
            # 3. Traffic-aware route
            routes['traffic_aware'] = self._optimize_traffic_aware(origin_node, destination_node, request)
            
            # 4. Multi-objective optimization
            routes['balanced'] = self._optimize_multi_objective(origin_node, destination_node, request)
            
            # Select best route based on preferences
            best_route = self._select_best_route(routes, request.preferences)
            
            # Generate alternative routes
            alternatives = [route for name, route in routes.items() if route != best_route][:request.max_alternatives]
            ...
            logger.info(f"Route optimization completed. Best route: {best_route.total_time_minutes:.1f} minutes")
            return best_route
    ...
    def _optimize_for_time(self, origin: str, destination: str, request: RouteOptimizationRequest) -> OptimizedRoute:
    ...
    def _optimize_for_distance(self, origin: str, destination: str, request: RouteOptimizationRequest) -> OptimizedRoute:
    ...
    def _optimize_traffic_aware(self, origin: str, destination: str, request: RouteOptimizationRequest) -> OptimizedRoute:
        ...
        if NETWORKX_AVAILABLE and self.road_network:
            ...
            def traffic_weight(u, v, edge_data):
                base_time = edge_data.get('current_time', edge_data.get('base_time', 10))
    ...
    def _optimize_multi_objective(self, origin: str, destination: str, request: RouteOptimizationRequest) -> OptimizedRoute:
        ...
        if NETWORKX_AVAILABLE and self.road_network:
            ...
            def multi_objective_weight(u, v, edge_data):
                ...
                time_cost = edge_data.get('current_time', edge_data.get('base_time', 10)) / 60  # hours
                distance_cost = edge_data.get('distance', 5) / 100  # normalized distance
                
                road_type = edge_data.get('road_type', 'local')
                comfort_factor = self.config['road_preferences'].get(road_type, {}).get('comfort', 1.0)
                fuel_efficiency = self.config['road_preferences'].get(road_type, {}).get('speed_factor', 1.0)
    ...
    def _build_route_from_path(self, path: List[str], optimization_type: str) -> OptimizedRoute:
    ...
    def _select_best_route(self, routes: Dict[str, OptimizedRoute], preferences: Dict[str, Any]) -> OptimizedRoute:
        ...
        for route_type, route in routes.items():
            ...
            if preferences.get('avoid_tolls', False):
                ...
                toll_penalty = sum(1 for segment in route.segments if segment.road_type == 'highway') * 5
                score -= toll_penalty
    ...
    def calculate_route_metrics(self, route: OptimizedRoute) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'traffic_metrics': {
                'average_congestion': np.mean([s.congestion_level for s in route.segments]),
                'average_traffic_factor': np.mean([s.traffic_factor for s in route.segments]),
                'congested_segments': len([s for s in route.segments if s.congestion_level > 0.5])
            },
            'road_composition': {},
    ...
    def _calculate_eco_score(self, route: OptimizedRoute) -> float:
        ...
        eco_score = max(0, base_score - distance_penalty - congestion_penalty - highway_penalty)
    ...
    try:
        ...
        if optimized_route.alternative_routes:
            ...
            for i, alt_route in enumerate(optimized_route.alternative_routes):
                print(f"  Alternative {i+1}: {alt_route.total_time_minutes:.1f} min, {alt_route.total_distance_km:.1f} km")
        
        # Calculate metrics
        metrics = optimizer.calculate_route_metrics(optimized_route)
        print(f"\nRoute Metrics:")
        print(f"  Average Speed: {metrics['basic_metrics']['average_speed_kmh']:.1f} km/h")
        print(f"  Average Congestion: {metrics['traffic_metrics']['average_congestion']:.2f}")
