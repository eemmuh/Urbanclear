...
class EnhancedTrafficPredictor:
    ...
    def prepare_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for window in [3, 6, 12, 24]:
                df[f'rolling_mean_{window}h'] = df['congestion_level'].rolling(window=window, min_periods=1).mean()
                df[f'rolling_std_{window}h'] = df['congestion_level'].rolling(window=window, min_periods=1).std()
            
            # Historical averages by time patterns
            df['historical_avg'] = df.groupby(['hour', 'day_of_week'])['congestion_level'].transform('mean')
            ...
            logger.info(f"Feature preparation completed. Features: {len(self.feature_columns)}")
            return df
    ...
    def train_models(self, data: pd.DataFrame, target_column: str = 'congestion_level') -> Dict[str, ModelPerformance]:
        ...
        try:
            ...
            logger.info(f"Model training completed. Trained {len(performances)} models.")
            return performances
    ...
    def _train_random_forest(self, X_train, X_test, y_train, y_test) -> ModelPerformance:
    ...
    def _train_linear_regression(self, X_train, X_test, y_train, y_test) -> ModelPerformance:
    ...
    def _create_ensemble_model(self, X_train, X_test, y_train, y_test) -> ModelPerformance:
    ...
    def predict(self, data: pd.DataFrame, model_type: str = 'ensemble', 
        ...
        try:
            ...
            confidence_interval = (
                max(0, prediction - 0.1),
                min(1, prediction + 0.1)
            )
