...
class OSMOverpassClient:
        ...
        try:
            ...
            async with self.session.post(url, data={"data": query}) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"OSM API request successful: {len(data.get('elements', []))} elements"
                    )
                    return data
                else:
        ...
        try:
            ...
            for element in result.get("elements", []):
                ...
                if element.get("type") == "node":
                    ...
                    lat, lon = element["center"].get("lat"), element["center"].get(
                        "lon"
                    )
                else:
                    continue
    ...
    async def search_places_by_name(
        self, name: str, lat: float, lon: float, radius_km: int = 10
    ) -> List[OSMPlace]:
    ...
    async def get_traffic_features(
        ...
        try:
            ...
            logger.info(
                f"Found traffic features: {len(traffic_features['traffic_signals'])} signals, "
...
async def search_osm_places(
    name: str, lat: float, lon: float, radius_km: int = 10
) -> List[OSMPlace]:
