...
class GeoapifyClient:
    ...
    async def geocode_address(self, address: str, limit: int = 10) -> List[GeoapifyGeocode]:
        ...
        try:
            ...
            for item in result.get("results", []):
                geocodes.append(GeoapifyGeocode(
    ...
    async def reverse_geocode(self, latitude: float, longitude: float) -> Optional[GeoapifyGeocode]:
    ...
    async def search_places(self, query: str, latitude: float, longitude: float,
                          radius_km: int = 10, categories: Optional[List[str]] = None,
                          limit: int = 20) -> List[GeoapifyPlace]:
        ...
        try:
            ...
            for item in result.get("results", []):
                ...
                places.append(GeoapifyPlace(
                    place_id=item.get("place_id", f"geo_{item.get('lat')}_{item.get('lon')}"),
    ...
    async def get_route(self, start_lat: float, start_lon: float,
                       ...
                       mode: str = "drive",
                       waypoints: Optional[List[Tuple[float, float]]] = None) -> Optional[GeoapifyRoute]:
    ...
    async def get_isochrone(self, latitude: float, longitude: float,
                          time_minutes: int = 15, mode: str = "drive") -> Optional[Dict[str, Any]]:
    ...
    async def autocomplete_address(self, text: str, latitude: float, longitude: float,
    ...
    async def get_places_by_category(self, latitude: float, longitude: float,
                                   categories: List[str], radius_km: int = 5,
                                   limit: int = 50) -> List[GeoapifyPlace]:
        ...
        try:
            ...
            for item in result.get("features", []):
                ...
                places.append(GeoapifyPlace(
                    place_id=properties.get("place_id", f"geo_{coordinates[1]}_{coordinates[0]}"),
...
async def geoapify_reverse_geocode(latitude: float, longitude: float) -> Optional[GeoapifyGeocode]:
    """Reverse geocode coordinates using Geoapify"""
    async with GeoapifyClient() as client:
        return await client.reverse_geocode(latitude, longitude)


async def geoapify_search_places(query: str, latitude: float, longitude: float,
                               radius_km: int = 10, limit: int = 20) -> List[GeoapifyPlace]:
    """Search places using Geoapify"""
    async with GeoapifyClient() as client:
        return await client.search_places(query, latitude, longitude, radius_km, limit=limit)


async def geoapify_route(start_lat: float, start_lon: float, end_lat: float, end_lon: float,
                        mode: str = "drive") -> Optional[GeoapifyRoute]:
