...
class OpenRouteServiceClient:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "No OpenRouteService API key provided. Service will be disabled."
            )
        ...
        try:
            ...
            if "geometry" in route_data:
                ...
                if isinstance(route_data["geometry"], str):
                    ...
                    geometry_coords = [
                        (coord[1], coord[0]) for coord in route_data["geometry"]
                    ]
    ...
    async def get_matrix(
        ...
        data = {"locations": coords, "metrics": metrics}
                ...
                pois.append(
                    {
                        ...
                        "category": properties.get("category_ids", {}).get(
    ...
    async def optimize_route(
        ...
        data = {"jobs": jobs, "vehicles": vehicles}
...
async def ors_route(
    coordinates: List[Tuple[float, float]], profile: str = "driving-car"
) -> Optional[ORSRoute]:
...
async def ors_matrix(
    locations: List[Tuple[float, float]], profile: str = "driving-car"
) -> Optional[ORSMatrix]:
