...
from .geoapify_client import GeoapifyClient, GeoapifyPlace, GeoapifyRoute, GeoapifyGeocode
from .openrouteservice_client import OpenRouteServiceClient, ORSRoute, ORSMatrix, ORSIsochrone
...
class RealDataService:
    ...
    async def geocode_address(self, address: str, prefer_source: Optional[str] = None) -> Optional[TrafficDataResult]:
        ...
        sources = [prefer_source] + self.geocoding_priority if prefer_source else self.geocoding_priority
                ...
                if source == "geoapify":
                    async with self.geoapify_client as client:
                        ...
                        if results:
                            ...
                            await self._set_cached_data(cache_key, result_data, 3600)  # Cache for 1 hour
                            return result
                    ...
                    if mock_result:
                        ...
                        await self._set_cached_data(cache_key, mock_result, 300)  # Cache for 5 minutes
                        return result
    ...
    async def get_route(self, start_lat: float, start_lon: float, 
                       ...
                       mode: str = "drive",
                       prefer_source: Optional[str] = None) -> Optional[RouteResult]:
        ...
        sources = [prefer_source] + self.routing_priority if prefer_source else self.routing_priority
        ...
        for source in sources:
            ...
            try:
                ...
                if source == "geoapify":
                    async with self.geoapify_client as client:
                        route = await client.get_route(start_lat, start_lon, end_lat, end_lon, mode)
                        if route:
                    ...
                    route_data = await self._generate_mock_route(start_lat, start_lon, end_lat, end_lon)
                ...
                if route_data:
                    ...
                    await self._set_cached_data(cache_key, cache_data, 1800)  # Cache for 30 minutes
                    return route_data
        ...
        logger.error(f"All routing sources failed for route: {start_lat},{start_lon} -> {end_lat},{end_lon}")
        return None
    
    async def search_places(self, query: str, latitude: float, longitude: float,
                          radius_km: int = 10, limit: int = 20,
                          prefer_source: Optional[str] = None) -> List[PlaceResult]:
        ...
        sources = [prefer_source] + self.places_priority if prefer_source else self.places_priority
        all_places = []
        ...
        for source in sources:
            ...
            try:
                ...
                if source == "geoapify":
                    async with self.geoapify_client as client:
                        geo_places = await client.search_places(query, latitude, longitude, radius_km, limit=limit)
                    ...
                    mock_places = await self._generate_mock_places(query, latitude, longitude, radius_km, limit)
                    if mock_places:
                        places.extend(mock_places)
                        logger.info(f"Found {len(mock_places)} mock places for '{query}'")
        ...
        if unique_places:
            ...
            await self._set_cached_data(cache_key, cache_data, 600)  # Cache for 10 minutes
        
        return unique_places
    
    async def get_traffic_matrix(self, locations: List[Tuple[float, float]],
                               prefer_source: Optional[str] = None) -> Optional[TrafficDataResult]:
            ...
            try:
                if source == "openrouteservice" and len(locations) <= 25:  # ORS limit
                    ...
                    if mock_matrix:
                        ...
                        await self._set_cached_data(cache_key, mock_matrix, 600)  # Cache for 10 minutes
                        return result
    ...
    async def get_isochrones(self, latitude: float, longitude: float,
                           time_minutes: List[float], mode: str = "drive",
                           prefer_source: Optional[str] = None) -> Optional[TrafficDataResult]:
                ...
                if source == "geoapify":
                    async with self.geoapify_client as client:
                        ...
                        for time_min in time_minutes:
                            iso = await client.get_isochrone(latitude, longitude, int(time_min), mode)
                            if iso:
                    ...
                    async with self.ors_client as client:
                        ...
                        isochrones = await client.get_isochrones(locations, time_minutes, "time", f"{mode}-car")
                    ...
                    mock_isochrones = await self._generate_mock_isochrones(latitude, longitude, time_minutes, mode)
                    if mock_isochrones:
                        result = TrafficDataResult(
    ...
    def _deduplicate_places(self, places: List[PlaceResult]) -> List[PlaceResult]:
        ...
        for place in places:
            ...
            for existing in unique_places:
                ...
                name_similarity = self._calculate_name_similarity(place.name, existing.name)
                
                if distance < 0.05 and name_similarity > 0.8:  # 50m and 80% name similarity
                    is_duplicate = True
                    break
    ...
    async def _generate_mock_places(self, query: str, latitude: float, longitude: float, 
                                  radius_km: int, limit: int) -> List[PlaceResult]:
        ...
        mock_places_db = {
            "coffee": [
                ...
                {"name": "Local Coffee Roasters", "categories": ["coffee", "cafe", "local"]},
                {"name": "The Daily Grind", "categories": ["coffee", "cafe", "breakfast"]},
                {"name": "Blue Bottle Coffee", "categories": ["coffee", "specialty"]},
                {"name": "Corner Cafe", "categories": ["coffee", "cafe", "quick"]},
            ],
            "restaurant": [
                {"name": "Tony's Italian Kitchen", "categories": ["restaurant", "italian"]},
                {"name": "Burger Palace", "categories": ["restaurant", "american", "fast food"]},
                {"name": "Sushi Zen", "categories": ["restaurant", "japanese", "sushi"]},
                {"name": "The Green Table", "categories": ["restaurant", "healthy", "vegetarian"]},
                {"name": "Pizza Corner", "categories": ["restaurant", "pizza", "italian"]},
            ],
            "gas": [
                {"name": "Shell Gas Station", "categories": ["gas station", "fuel"]},
                {"name": "BP Service Station", "categories": ["gas station", "fuel", "convenience"]},
                {"name": "Chevron", "categories": ["gas station", "fuel"]},
                {"name": "ExxonMobil", "categories": ["gas station", "fuel", "convenience"]},
                {"name": "Local Fuel Stop", "categories": ["gas station", "fuel", "local"]},
            ],
            "hotel": [
            ...
            "store": [
                {"name": "SuperMart", "categories": ["store", "grocery", "supermarket"]},
                {"name": "Corner Store", "categories": ["store", "convenience"]},
                {"name": "Electronics Plus", "categories": ["store", "electronics"]},
                {"name": "Fashion Boutique", "categories": ["store", "clothing", "fashion"]},
                {"name": "Hardware Store", "categories": ["store", "hardware", "tools"]},
            ]
        }
        ...
        for category, places in mock_places_db.items():
            if category in query_lower or any(cat in query_lower for cat in ["shop", "store", "restaurant", "cafe"]):
        ...
        for i, place_template in enumerate(matching_places[:limit]):
            ...
            lat_offset = (distance / 111.0) * random.choice([-1, 1])  # 111 km per degree latitude
            ...
            actual_distance = self._calculate_distance(latitude, longitude, mock_lat, mock_lon)
            ...
            result = PlaceResult(
                ...
                properties={
                    "mock": True,
                    "opening_hours": "9:00-21:00" if "restaurant" in place_template["categories"] else "24/7",
    ...
    async def _generate_mock_matrix(self, locations: List[Tuple[float, float]]) -> Dict[str, Any]:
    ...
    async def _generate_mock_isochrones(self, latitude: float, longitude: float, 
                                      time_minutes: List[float], mode: str) -> List[Dict[str, Any]]:
        ...
        for time_min in time_minutes:
            ...
            for i in range(num_points + 1):  # +1 to close the polygon
                ...
                point_lon = longitude + radius_degrees * math.sin(angle) / math.cos(math.radians(latitude))
    ...
    async def _generate_mock_geocoding(self, address: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            "latitude": 40.7589 + (random.random() - 0.5) * 0.1,  # Random point around NYC
    ...
    async def _generate_mock_route(self, start_lat: float, start_lon: float, 
                                 end_lat: float, end_lon: float) -> RouteResult:
        """Generate mock route as fallback"""
        distance = self._calculate_distance(start_lat, start_lon, end_lat, end_lon) * 1000  # Convert to meters
        ...
        for source_name in ["openstreetmap", "geoapify", "openrouteservice"]:
            try:
                ...
                status["sources"][source_name] = {
                    ...
                    "rate_limit_remaining": self.config.get_remaining_requests(source_name)
                }
