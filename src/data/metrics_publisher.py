...
class MetricsPublisher:
        ...
        try:
            ...
            for sensor in traffic_data["sensors"]:
                ...
                self.traffic_flow_rate.labels(
                    location=location, direction="northbound"
                ).set(sensor["flow_rate"] * random.uniform(0.8, 1.2))

                self.traffic_flow_rate.labels(
                    location=location, direction="southbound"
            ...
            for condition in weather_conditions:
                ...
                self.weather_impact_factor.labels(weather_condition=condition).set(
                    impact
                )
    ...
    async def _process_traffic_metrics(self, data: List[Dict[str, Any]]) -> None:
        ...
        for item in data:
            ...
            await self._publish_speed_metrics(item)
            await self._publish_congestion_metrics(item)
    ...
    def stop(self):
        ...
        if self.loop:
            asyncio.run_coroutine_threadsafe(
                self.publisher.stop_publishing(), self.loop
            )
