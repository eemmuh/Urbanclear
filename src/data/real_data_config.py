...
class RealDataConfig:
    ...
    def _initialize_data_sources(self):
        ...
        self.data_sources["openstreetmap"] = DataSourceConfig(
            ...
            credentials=APICredentials(
                ...
                headers={
                    "User-Agent": "Urbanclear-Traffic-System/1.0 (contact@urbanclear.com)"
                },
            ),
            rate_limits=RateLimit(
    ...
    def get_sources_for_type(
        self, data_type: DataSourceType, enabled_only: bool = True
    ...
    def can_make_request(self, source_name: str) -> bool:
        ...
        return (
            rate_limit.minute_count < rate_limit.requests_per_minute
            and rate_limit.hour_count < rate_limit.requests_per_hour
            and rate_limit.day_count < rate_limit.requests_per_day
            and rate_limit.month_count < rate_limit.requests_per_month
        )
    ...
    def get_cost_estimate(self, source_name: str, requests: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "free_tier_remaining": max(
                0,
                source.rate_limits.requests_per_month - source.rate_limits.month_count,
            ),
        }
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        for name, source in self.data_sources.items():
            source_status = {
                ...
                "has_credentials": (
                    bool(source.credentials.api_key)
                    if source.tier != APITier.FREE
                    else True
                ),
                "tier": source.tier.value,
            ...
            if (
                source.tier in [APITier.FREEMIUM, APITier.PREMIUM]
                and not source.credentials.api_key
            ):
        ...
        # Add recommendations
        if not any(
            s.tier == APITier.PREMIUM for s in self.data_sources.values() if s.enabled
        ):
            status["recommendations"].append(
                "Consider adding a premium API for better traffic data quality"
            )

        if not os.getenv("GOOGLE_MAPS_API_KEY"):
            status["recommendations"].append(
                "Add GOOGLE_MAPS_API_KEY for premium traffic data"
            )
