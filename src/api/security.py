...
class SecurityManager:
    ...
    def create_access_token(self, user: User) -> str:
        ...
        payload = {
            ...
            "exp": datetime.utcnow() + timedelta(hours=self.security_config.JWT_EXPIRATION_HOURS),
            "iat": datetime.utcnow(),
            "jti": hashlib.sha256(f"{user.id}:{datetime.utcnow()}".encode()).hexdigest()[:16]
        }
        
        token = jwt.encode(payload, self.security_config.JWT_SECRET_KEY, algorithm=self.security_config.JWT_ALGORITHM)
    ...
    def generate_api_key(self, user_id: str, description: str = "") -> str:
        ...
        api_key = self.security_config.API_KEY_PREFIX + hashlib.sha256(key_data.encode()).hexdigest()
...
ROLE_PERMISSIONS = {
    ...
    UserRole.TRAFFIC_MANAGER: [
        "read:traffic", "write:traffic", "read:signals", "write:signals", 
        "read:incidents", "write:incidents", "read:analytics"
    ],
    UserRole.ANALYST: [
        "read:traffic", "read:analytics", "read:predictions", "read:incidents"
    ],
    UserRole.VIEWER: [
        "read:traffic", "read:analytics"
    ],
    UserRole.API_USER: [
        "read:traffic", "read:predictions", "read:routes"
    ]
}
...
def require_role(role: UserRole):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if user.get('role') != role.value and user.get('role') != UserRole.ADMIN.value:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
