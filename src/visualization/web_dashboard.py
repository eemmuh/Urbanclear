...
class WebDashboard:
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    y=flow_rates,
                    marker_color=[
                        "red" if rate > 600 else "orange" if rate > 400 else "green"
    ...
    def create_congestion_map(self, traffic_data: Dict) -> go.Figure:
        ...
        if not traffic_data or "sensors" not in traffic_data:
            ...
            locations = [sensor["location"] for sensor in traffic_data["sensors"]]
            lats = [
                sensor.get("coordinates", {}).get("lat", 40.7580)
                for sensor in traffic_data["sensors"]
            ]
            lons = [
                sensor.get("coordinates", {}).get("lon", -73.9855)
                for sensor in traffic_data["sensors"]
            ]
            congestion = [
                sensor.get("congestion_level", 0.5)
                for sensor in traffic_data["sensors"]
            ]
        ...
        fig = go.Figure(
            data=go.Scattermapbox(
                ...
                text=[
                    f"{loc}<br>Congestion: {c:.2f}"
                    for loc, c in zip(locations, congestion)
                ],
        ...
        fig.update_layout(
            ...
            mapbox=dict(
                center=go.layout.mapbox.Center(lat=40.7589, lon=-73.9851), zoom=11
            ),
            showlegend=False,
            height=400,
    ...
    def create_incidents_chart(self, incidents_data: Dict) -> go.Figure:
        ...
        fig.update_layout(title="🚨 Active Incidents by Severity", height=400)
    ...
    while auto_refresh:
        with placeholder.container():
            ...
            with col1:
                ...
                st.plotly_chart(
                    traffic_fig, use_container_width=True, key="traffic_flow_chart"
                )

                # Predictions chart
                prediction_fig = dashboard.create_prediction_chart(prediction_data)
                st.plotly_chart(
                    prediction_fig, use_container_width=True, key="prediction_chart"
                )
            ...
            with col2:
                ...
                st.plotly_chart(
                    map_fig, use_container_width=True, key="congestion_map_chart"
                )

                # Incidents chart
                incidents_fig = dashboard.create_incidents_chart(incidents_data)
                st.plotly_chart(
                    incidents_fig, use_container_width=True, key="incidents_chart"
                )
